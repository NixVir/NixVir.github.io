...
The frontend loads both automated and manual news feeds via JavaScript:

```javascript
async function loadSkiNews() {
  const [autoResponse, manualResponse] = await Promise.all([
    fetch('/data/ski-news.json'),
    fetch('/manual-news/index.json').catch(() => null)
  ]);
  const data = await autoResponse.json();

  // Merge manual stories (CMS-managed) if any exist
  if (manualResponse && manualResponse.ok) {
    const manualData = await manualResponse.json();
    // Filter active stories, check pin expiration, prepend to feed
    data.articles = [...manualArticles, ...(data.articles || [])];
  }
...
| `layouts/page/ski-news.html` | Ski news page template (loads both feeds) |
| `layouts/index.html` | Front page (also loads both feeds) |
...
- Title similarity threshold at 85% with source suffix stripping (`_strip_source_suffix()`)
- Strips " - Source Name" suffixes before comparing, catching Google News aggregator variants
- Added lead paragraph comparison to catch same story with different headlines
- Front page normalizes all "Google News - *" sources as one source for dedup
- Front page checks title prefix similarity (40+ shared chars = duplicate)
...
### Manual News Stories (CMS) — February 2026

Manual stories can be added via Sveltia CMS at `/admin/` (collection: "Ski News — Manual Stories"). Each story is a markdown file in `content/manual-news/` with frontmatter fields for title, URL, source, category, content type, description, pub date, pin duration, and active status.

**Pipeline**: CMS save → git commit → Netlify build → `/manual-news/index.json` generated by Hugo → fetched by both ski-news page and front page.

**Pin system**: Stories stay pinned at top of feed for a configurable duration (1-30 days). Pin expiration is calculated at build time (`pub_date + pin_days`) and verified at display time.

**Both pages**: `layouts/page/ski-news.html` and `layouts/index.html` fetch and merge manual stories into the automated feed. The front page also applies Google News source normalization and title prefix similarity checking for deduplication.
